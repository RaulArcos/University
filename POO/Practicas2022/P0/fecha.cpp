//Fecha.cpp Raúl Arcos Herrera

#include "fecha.hpp"

//Constructor Fecha a partir de INTs
Fecha::Fecha(int d, int m, int a):dia_(d),mes_(m),anno_(a) //Asignamos valores a dia_,mes_ y anno_
{
...
/*
*Este es el constructor para una fecha a partir de otra fecha, pero la opción más óptima fue
utilizar el construcctor "default" en fecha.hpp*
...
//Procedimiento para actualizar una fecha a la que se le realiza una operación.
/*
void Fecha::actualiza()
{   
   
    static int dias_mes[] = {0,31,28,31,30,31,30,31,31,30,31,30,31};
   // int Bisiesto = (anno_ % 4 == 0 && (anno_ % 100 != 0 || anno_ % 400 == 0));

    //                2, 1, 2010 -2 dia = 0 mes = 0
    
    if(dia_>dias_mes[mes_])
    {
        dia_ -= dias_mes[mes_] ;mes_++;
    } 
    if(dia_<1)
    {
        dia_ += dias_mes[mes_]; mes_--;
    ...
    if(mes_ < 1)
    {
        mes_ = 12; anno_--; dia_ = dias_mes[mes_];
    } 
...
bool operator == (const Fecha& Fecha1, const Fecha& Fecha2) //Se comprueban si dia,mes y año son iguales.
{
    return ((Fecha1.dia() == Fecha2.dia()) && (Fecha1.mes() == Fecha2.mes()) && (Fecha1.anno() == Fecha2.anno()));
}
bool operator != (const Fecha& Fecha1, const Fecha& Fecha2) //Devolvemos el resultado contrario a ==.
{
    return !(Fecha1 == Fecha2);
}
bool operator > (const Fecha& Fecha1, const Fecha& Fecha2) //Comprobamos desde año hasta dia si Fecha1 es mayor que Fecha2
{
    ...
    else if(Fecha1.anno() == Fecha2.anno() && Fecha1.mes() == Fecha2.mes() && Fecha1.anno() == Fecha2.anno() && Fecha1.dia() > Fecha2.dia())
    {
        return 1;
    }
    else return 0;
}
bool operator < (const Fecha& Fecha1, const Fecha& Fecha2) //Comparamos las fechas del revés.
{
    return(Fecha2 > Fecha1);
}
bool operator >= (const Fecha& Fecha1, const Fecha& Fecha2) //Utilizamos los operadores > y == ya hechos.
{
...
istream& operator >>(istream& inputbuffer, Fecha& f)
{
    ...
    {   
        inputbuffer.setstate(ios::failbit);
        delete[] cad;
        throw e;
