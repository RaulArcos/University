...
Cadena Cadena::substr(unsigned indice, unsigned tam) const{
    ...
    {
        throw out_of_range("substr -> Fuera de rango");
    }
...
char& Cadena::at(size_t indice)
{
    ...
    throw out_of_range("at -> Error, indice > tamaño o indice < 0");
}
...
// Operador de índice
const char& Cadena::operator[](const size_t indice) const noexcept // Consultora
{
    return this->s_[indice] ;
}

char& Cadena::operator[] (const size_t indice) noexcept // Modificadora
{
    return this->s_[indice] ;
}
...
istream& operator >>(istream& inputbuffer, Cadena& cad)
{
    char c;
...
ostream& operator <<(ostream& outputbuffer, const Cadena& cad)
{
