...
vector<tCoste> DijkstraInv(const GrafoP<tCoste>& G,
   ...
   //Esta parte la dejamos igual para iniciar el vector S y D.
   typedef typename GrafoP<tCoste>::vertice vertice;
   vertice v, w;
   const size_t n = G.numVert();
   vector<bool> S(n, false);    // Conjunto de vértices vacío.
   vector<tCoste> D(n);   
   //LLenamos D con las distancias desde cada vértice hasta el destino.
    for(int i = 0; i < n; ++i )
      D[i] = G[i][destino];
   D[destino] = 0;                             // Coste destino-destino es 0.
   P = vector<vertice>(n, destino);
   //Calculamos el camino mínimo desde cada vértice al destino
   for(int i = 1; i < n-1; ++i){
      //cogemos un vertice (w) que no esté en S y con el menor coste hacia destino.
      tCoste costeMin = GrafoP<tCoste>::INFINITO;
      for(v = 0; v < n; v++)
         if(!S[v] && D[v] <= costeMin){
            costeMin = D[v];
